...
// Define interface for Cloudinary API response
...
const Cloudinary: React.FC = () => {
    ...
    // Define the event handler with proper TypeScript type
    const uploadImage = async (e: ChangeEvent<HTMLInputElement>): Promise<void> => {            //2 Preparamos para recibir el evento al ejecutarse la función async
        const files = e.target.files            //3 recuperamos el array de e.target.files
        if (!files || files.length === 0) return; // TypeScript safety check
        ...
        try {
            ...
            const file = await response.json() as CloudinaryResponse;     //11 Traducimos la respuesta de JSON
